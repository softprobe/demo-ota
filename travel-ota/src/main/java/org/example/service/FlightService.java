...
public class FlightService {
    ...
    /** 退票手续费回归开关:开启后中间档下界从 24h 抬到 48h(24-48h 退票从 30% 掉进 50% 档) */
    private static final String REFUND_REGRESSION_ENV = "SP_DEMO_REFUND_REGRESSION";
    ...
    /**
     * 退票:JPA 查订单 → 在 travel-ota 本地算手续费(带回归开关)→ 调 sp-airline
     * /refund/process → 用本地算出的手续费/净退款覆盖响应 → JPA 置 REFUNDED → 失效缓存。
     *
     * <p>手续费算在 travel-ota 自己这里(不转发给 sp-airline 算),这样回放本接口时
     * {@link #REFUND_REGRESSION_ENV} 开关引入的差异会直接暴露在 travel-ota 的响应里。
     */
    public RefundResponse processRefund(RefundRequest request) {
        ...
        // HTTP —— 调 sp-airline 处理退票(下游 span)
    ...
    /**
     * 退票手续费。正确版本中间档下界 24h;回归版本抬到 48h,使 24-48h 的退票从
     * 30% 档掉进 50% 档,多扣 20%。逻辑照 sp-airline 原实现,并叠加 env 开关。
     */
    private BigDecimal calculateCancellationFee(long hoursUntilDeparture, String refundReason, BigDecimal totalAmount) {
        ...
        // 个人原因 —— 按距起飞时长分档
        long midTierLowerBound = isRefundRegressionEnabled() ? 48 : 24;
        if (hoursUntilDeparture > 72) {
            return scale(totalAmount.multiply(new BigDecimal("0.15"))); // 15%
        } else if (hoursUntilDeparture > midTierLowerBound) {
    ...
    private boolean isRefundRegressionEnabled() {
        String v = System.getenv(REFUND_REGRESSION_ENV);
        return v != null && ("true".equalsIgnoreCase(v.trim())
                || "1".equals(v.trim()) || "on".equalsIgnoreCase(v.trim()));
    }
